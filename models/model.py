    ...
    def load_networks(self, epoch):
        ...
        for name in ['G', 'D']:
            if isinstance(name, str):
                ...
                net.set_state_dict(paddle.load(load_path))
                # if self.gpu_ids and paddle.cuda.is_available():
    ...
    def sample(self, batch, basename):
        ...
        with paddle.no_grad():
            ...
            for label, image_tensor in zip(batch[0], tensor_to_plot):
                ...
                chk_mkdir(label_dir)
                save_image(image_tensor, os.path.join(label_dir, str(cnt) + '.png'))
                cnt += 1
            ...
            self.set_input(paddle.randn(1, self.embedding_num).repeat(batch[0].shape[0], 1), batch[2], batch[1])
            self.forward()
            tensor_to_plot = paddle.cat([self.fake_B, self.real_A], 3)
